...
public class QuestionController {
    ...
    public BaseResponse<Long> doQuestionSubmit(@RequestBody QuestionSubmitAddRequest questionSubmitAddRequest, HttpServletRequest request) {
        ...
        // 提交数据增加
        boolean submitNumResult = questionService.updateSubmitNum(question);
        if (!submitNumResult){
            log.info("提交数目添加失败");
        ...
        // 通过数目增加
        boolean acceptedNumResult = questionSubmitService.updateAcceptedNum(question,questionSubmitId);
        if (!acceptedNumResult){
            log.info("通过数目添加失败");
            throw new BusinessException(ErrorCode.OPERATION_ERROR);
        }
    ...
    //根据  提交题目id 查询 查询当前提交题目信息
    @GetMapping("/question_submit/get")
    public BaseResponse<QuestionSubmit> getQuestionSubmitVoById(Long id){
        log.error("getQuestionSubmitVoById");
